              		...
              		if(result==0)
               		{
           	 	  		try
           	 	  		{
           	 	  		    set=st.executeQuery("select count(*) as me from Table2 where Username='"+txt1.getText()+"'");
           	 	  		    set.next();
           	 	  		    if(Integer.parseInt(set.getString("me"))==0)JOptionPane.showMessageDialog(fr,"User Not Found !");
           	 	  		    else 
           	 	  		    {
           	 	  		    	set=st.executeQuery("select password from Table2 where Username='"+txt1.getText()+"'");
           	 	  		        set.next();
           	 	  		        if(!set.getString("password").equals(pass.getText()))JOptionPane.showMessageDialog(fr,"Incorrect Password Please Try Again !");
           	 	  		        else {
           	 	  		        	     set=st.executeQuery("select * from Table2 where Username='"+txt1.getText()+"'");
           	 	  		                 set.next();
